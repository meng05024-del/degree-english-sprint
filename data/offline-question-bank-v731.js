...
globalThis.offlineQuestionBankV731=(()=>{
  ...
  flatWords.forEach((w,i)=>{
    ...
    add({id:`v731-word-read-${w.id}`,type:'listening',cat:'词汇听读',q:`辅助读法“${w.read}”对应的英文单词是：`,options:pick(flatWords,i,'en'),answer:0,point:`读音与拼写：${w.en}`,explain:`“${w.read}”是 ${w.en} 的暂时辅助读法，意思是“${w.cn}”。请以应用内标准语音为准。`,translation:'根据辅助读法辨认英文拼写。',vocab:[w],ref:{type:'word',id:w.id},difficulty:'巩固'});
  });
  ...
  plurals.forEach((noun,i)=>{
    add({id:`v731-be-plural-${i}`,type:'grammar',cat:'be动词',q:`These ${noun} ___ new.`,options:['are','is','am','be'],answer:0,point:'复数主语用 are',explain:`These 和复数名词 ${noun} 构成复数主语，因此使用 are。`,translation:`这些${noun}是新的。`,difficulty:'基础'});
    add({id:`v731-plural-${i}`,type:'grammar',cat:'名词复数',q:`There are two ___ here.`,options:[noun,singulars[i],`${singulars[i]}es`,`${singulars[i]}'s`],answer:0,point:'数量大于一时用名词复数',explain:`two 表示两个，后面需要使用复数名词 ${noun}。`,translation:`这里有两个${singulars[i]}。`,difficulty:'基础'});
  ...
  const possessives=[['my','I','我的'],['your','you','你的'],['his','he','他的'],['her','she','她的'],['our','we','我们的'],['their','they','他们的'],['my','I','我的'],['your','you','你们的']];
  possessives.forEach((x,i)=>add({id:`v731-possessive-${i}`,type:'grammar',cat:'代词',q:`This is ___ ${i%2?'book':'friend'}.`,options:[x[0],x[1],i%2?'her':'they',i%2?'we':'she'],answer:0,point:'形容词性物主代词放在名词前',explain:`名词前要用表示“谁的”的词，${x[0]} 表示“${x[2]}”。`,translation:`这是${x[2]}${i%2?'书':'朋友'}。`,difficulty:'基础'}));
  ...
  const modals=[['can','read English','会读英语'],['can','write this word','会写这个单词'],['can','help you','能帮助你'],['can','speak English','会说英语'],['will','study tomorrow','明天会学习'],['will','work hard','会努力工作'],['must','finish the test','必须完成测试'],['should','review today','今天应该复习']];
  modals.forEach((x,i)=>add({id:`v731-modal-${i}`,type:'grammar',cat:'情态动词',q:`I ${x[0]} ___ .`,options:[x[1],`${x[1]}s`,`${x[1]}ing`,`to ${x[1]}`],answer:0,point:'情态动词后使用动词原形',explain:`${x[0]} 是情态动词，后面直接接动词原形：${x[0]} ${x[1]}。`,translation:`我${x[2]}。`,difficulty:'提高'}));
