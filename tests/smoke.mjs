...
try {
  ...
  assert.match(await page.locator('.course-entry').innerText(),/先学题型，再做题[\s\S]*12 类必考基础/);
  await page.locator('#open-topic-courses').click();
  assert.match(await page.locator('.topic-header h1').innerText(),/零基础题型课/);
  assert.equal(await page.locator('.topic-row').count(),12);
  ...
  assert.match(await page.locator('.course-entry').innerText(),/已学 1\/12/);
  ...
  assert.match(await page.locator('.analysis-pagination').innerText(),/1 \/ 40/);
  await page.screenshot({path:'test-output/analysis-center-mobile.png',fullPage:true});
  await page.locator('#analysis-next').click();
  assert.match(await page.locator('.analysis-pagination').innerText(),/2 \/ 40/);
  ...
  assert.equal(wrongAudit.version,'DEGREE-ENGLISH-WEB-V4.7');
