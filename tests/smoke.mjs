...
try {
  ...
  const contentAudit = await page.evaluate(() => {
    ...
    const stems=bank.map(q=>String(q.q||'').replace(/\s+/g,' ').trim().toLowerCase());
    return {
      total:bank.length,
  ...
  assert.equal(contentAudit.total,774);
  ...
  const audit = await page.evaluate(() => {
    ...
    for (const [index,id] of state.questionIds.entries()) {
      ...
      stems.add(q.q.replace(/\s+/g,' ').trim().toLowerCase());
  ...
  assert.equal(wrongAudit.version,'DEGREE-ENGLISH-WEB-V4');
  ...
  const reinforceAudit=await page.evaluate(() => {
    const state=JSON.parse(localStorage.getItem('degree_english_50_quiz_v1'));
    const bank=globalThis.offlineQuestionBankV731;
    const stems=state.questionIds.map(id=>bank.find(q=>q.id===id).q.replace(/\s+/g,' ').trim().toLowerCase());
  ...
  console.log(JSON.stringify({status:'passed',contentAudit,audit,history:2,scoreChecks:[100,0],wrongReport:wrongAudit,reinforcement:reinforceAudit,mastery:masteryAudit}));
} finally {
  await browser.close();
}
