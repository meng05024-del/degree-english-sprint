...
try {
  ...
  assert.match(await page.locator('.course-entry').innerText(),/先学题型，再做题[\s\S]*44 节小课/);
  await page.locator('#open-topic-courses').click();
  assert.match(await page.locator('.topic-header h1').innerText(),/零基础题型课/);
  assert.equal(await page.locator('.topic-row').count(),44);
  ...
  await page.locator('[data-topic="predicate"]').click();
  assert.match(await page.locator('.topic-header h1').innerText(),/谓语是什么/);
  assert.match(await page.locator('.term-list').innerText(),/谓语动词[\s\S]*复合谓语[\s\S]*系表结构/);
  assert.match(await page.locator('.lesson-confusions').innerText(),/I happy[\s\S]*will go/);
  assert.equal(await page.locator('.lesson-extra-examples article').count(),3);
  assert.match(await page.locator('.lesson-memory').innerText(),/发动机/);
  await page.screenshot({path:'test-output/predicate-lesson-mobile.png',fullPage:true});
  await page.locator('#topic-back').click();
  await page.locator('#topic-home').click();
  assert.match(await page.locator('.course-entry').innerText(),/已学 1\/44/);
  ...
  assert.equal(wrongAudit.version,'DEGREE-ENGLISH-WEB-V4.9');
